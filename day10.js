...
const testInstructions = '3,4,1,5'
const test = testInstructions.split(',').map(Number)
...
function knot({list, pos, skip}, len) {
  const listLen = list.length
  const section = []
  // reverse
  for (let i = 0; i < len; i++) {
    section[len-i-1] = list[(pos + i) % listLen]
  }
  //refill list
  for (let i = 0; i < len; i++) {
    list[(pos + i) % listLen] = section[i]
  }
  return {
    list,
    pos: (pos + len + skip) % listLen,
    skip: skip + 1
  }
}

//const test1 = test.reduce(knot, {list: getList(5), pos: 0, skip: 0})

const input = require('fs').readFileSync('./input10.txt', {encoding: 'UTF8'}).split(',').map(Number)

const part1 = input
  .reduce(knot, {list: getList(256), pos: 0, skip: 0})
  .list.slice(0,2)
  .reduce((res, n) => n * res, 1)
...
the sequence: 17, 31, 73, 47, 23. For example, if you are given 1,2,3, your final sequence of
...
between rounds. For example, if the previous example was your first round, you would start your
second round with the same length sequence (3, 4, 1, 5, 17, 31, 73, 47, 23, now assuming they
came from ASCII codes and include the suffix), but start with the previous round's current
...
in the sparse hash (there are 16 such blocks in a list of 256 numbers). So, the first element
in the dense hash is the first sixteen elements of the sparse hash XOR'd together, the second
...
output is the dense hash in hexadecimal notation. Because each number in your dense hash will
be between 0 and 255 (inclusive), always represent each number as two hexadecimal digits (including
a leading zero as necessary). So, if your first three numbers are 64, 7, 255, they correspond
to the hexadecimal numbers 40, 07, ff, and so the first six characters of the hash would be
...
const input2 = fs.readFileSync('./input10.txt', {encoding: 'UTF8'}).split('')
  .map(c => c.charCodeAt())
...
for (let i=0; i < 64; i++) {
  answer = input2.reduce(knot, answer)
}

const sparse = answer.list
const dense = []
for (let i = 0; i < 16; i++) {
  let char = 0
  for (let j = 0; j < 16; j++) {
    char = char ^ sparse[i * 16 + j]
  }
  dense.push(char)
}

const part2 = Buffer.from(dense).toString('hex')
