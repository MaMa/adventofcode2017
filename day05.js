...
const input = readLines('./input05.txt').map(Number)

function maze1(commands) {
  let cmds = commands.slice()
  let ptr = 0
  let cnt = 0
  while (typeof cmds[ptr] !== 'undefined') {
    cnt += 1
    const cmd = cmds[ptr]
    cmds[ptr] += 1
    ptr += cmd
...
console.log('test1', maze1(test))
console.log('Part1', maze1(input))
...
console.log('test2', maze2(test))
console.log('Part2', maze2(input))
